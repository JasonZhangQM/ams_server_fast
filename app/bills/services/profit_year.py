...
def upsert_profit_year_sql():
    ...
    通过 JOIN bills_bill 取 trade_time 提取年份，对 pl_long/pl_short/pl_other
    按 year 分组求和，pl_all 为三者合计，pl_cumulative 为按年升序的累计求和。
    空数据时跳过 UPSERT。
    """
    ...
    sql = f'''
        SELECT YEAR(b.trade_time) AS year,
               p.pl_long, p.pl_short, p.pl_other
        FROM bills_profit p
    ...
    # 需先转为 float 再聚合
    numeric_cols = ['pl_long', 'pl_short', 'pl_other']
    df[numeric_cols] = df[numeric_cols].apply(pd.to_numeric, errors='coerce').fillna(0)
    # 按 year 分组求和
    df_year = df.groupby('year', as_index=False)[numeric_cols].sum()
    # pl_all = pl_long + pl_short + pl_other
    df_year['pl_all'] = (
