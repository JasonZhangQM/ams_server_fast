# -*- coding: utf-8 -*-
"""每日估值同步：gm 接口获取估值指标（PE/PB/PCF/PS/PEG/DY）并入库。

与三大财报的差异：
- 数据源仅 gm stk_get_daily_valuation，无 pub_date 保留策略
- 按 (symbol, trade_date) 直接覆盖更新
- 共用 fund_data 的 _clean_scalar/_to_date 工具函数
"""
import logging
...
from server_fast.common.utils import call_with_timeout, upsert_df_to_db
from server_fast.common.db import SessionLocal
from server_fast.app.bds.models import DailyValuation
...
def upsert_daily_valuation_sql(symbols):
    ...
    """
    _mdl = DailyValuation
    _field_list = DAILY_VALUATION_FIELDS.split(",")
    steps = {}
