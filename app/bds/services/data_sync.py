...
涵盖：
- 交易日历、证券信息、指数历史、指数成分（基础市场数据）
- 资产负债表、利润表、现金流量表（三大财报数据）
- 财务指标、估值指标（衍生财务数据）

数据源：gm API（终档终端），通过 pandas + settings.DB_ENGINE 入库。
"""
import pandas as pd
import numpy as np
import logging
...
def _upsert_fund_sql(symbols, model, fields_str, api_func, log_name):
    ...
    """
    _mdl = model
    _field_list = fields_str.split(",")  # 20 个财务字段名列表
